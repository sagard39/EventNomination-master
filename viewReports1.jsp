...
function add_new(){
	document.form1.action_type.value="add_new";
		document.form1.submit();
}

function send_to_sap(){
	document.form1.action_type.value="Send_to_SAP";
		document.form1.submit();
	
}

function delete_nom(emp){
	if(confirm("YOU WANT TO CANCEL THIS NOMINATION? IF YES THEN CLICK ON OK BUTTON")){
		var delemp=emp;
		document.form1.emp.value = delemp;
		document.form1.action_type.value = "Delete";	
		document.form1.submit();
	}
}
...
if("Delete".equals(action_type)){
			...
			psEvt=con.prepareStatement("update nomination set flag='X' where evtnme=? and emp_no=?");
			...
			<script>
				...
				document.location.href = "viewReports1.jsp";
			</script>
			<%
			}
}

if("add_new".equals(action_type)){
	%><script>
		window.location.href="mainSubmit_admin.jsp?evtnme=<%=evt_id%>";
	</script><%
}

if("Send_to_SAP".equals(action_type)){

	%><script>
		window.location.href="send_to_sap.jsp?evtnme=<%=evt_id%>";
	</script><%
}
...
<div class="card-header alert alert-primary style-app-name"><h3 class="text-white">Event Reports </h3></div>
<div class="card-body">
	<div class="row">
		<div class="col-sm-12">
			<p class="style-text-red"><marquee><b>Recovery for the month of June to be processed in JDE and after that in SAP.</b></marquee></p>
		</div>
	</div>
<div class = "row">
...
<%if("".equals(view)){%>		
		<div class="row">
			<div class="col-md-3" >
				...
				<option	value="<%=entry.getKey()%>" <%=evt_id.equals(entry.getKey())?"selected":""%>><%=entry.getValue()%></option>	
				<%}
				%>
				</select></span>
			</div>
			...
			<div class="col-md-3"><a href="viewReports1.jsp?q=All">Click Here for Standard Report</a>
				<input type="submit" class="style-right-button" name="submitRept" value="Submit" onclick="return submitForm1();"/></center>
			</div>
		</div>
<!--<div class="row">
...
String queryAddtionalField="select lbl_name,upper(lbl_type) lbl_type,id from NOMINATION_ADDITION where  evt_id=? and level_type='E' AND status='A'";	
String queryAdditionlFieldValue="select nvl(value,' ')value,id from NOMINATION_EMP_ADD where id in(select id from NOMINATION_ADDITION where evt_id=? and level_type='E' and status='A') and emp_no=?";

String queryAddtionalField1="select QUESTIONNAIRE,upper(QUESTION_TYPE) QUESTION_TYPE,id from NOMINATION_QUESTIONNAIRE where  evt_id=? and status='A' order by id asc";
String queryAdditionlFieldValue1="select nvl(value,' ')value,id from NOMINATION_EMP_ADD where id in(select id from NOMINATION_QUESTIONNAIRE where evt_id=? and STATUS='A' ) and emp_no=? order by id asc";
...
PreparedStatement psAdditionalValue1=null;
ResultSet rsAdditionalValue1=null;
ResultSet rs1=null;
townName=nullVal(request.getParameter("locn"));
String whereClause  = "where flag = 'S'";
boolean isEmpty=false;
if(!"".equals(evt_id)){
	whereClause += " and evt_id ='"+evt_id+"'";
} if(!"".equals(dept)){
	whereClause += "and e.bu = '"+dept+"'";
} if(!"".equals(townName)){
	whereClause += "and e.town = '"+townName+"'";
}
queryChild = "select n.evtnme,na.evtnme event_nme,n.emp_no,e.town,e.budesc,(select emp_name from empmaster where emp_no=n.emp_no) emp_name,n.TICKTSCNT,n.paymentcnt,to_char(enterdte,'dd-Mon-YYYY HH24:MI') enterdate1,enterdte,rownum,to_char(modifydte,'dd-Mon-YYYY HH24:MI') modifydte,modifydte modifydte1 , TO_CHAR(na.cutofdte,'yyyymmdd') as chk_evt_dt from nomination n left join nomination_admin na on n.evtnme=na.evt_id left join empmaster e on n.emp_no=e.emp_no "+whereClause+" order by modifydte1 desc ";
...
%>		<div class="row">
	...
	if("2018_126_event".equals(evt_id)){
		...
		query="select Sum(Ticktscnt) countTickets,count(distinct emp_no) empCount ,decode(flag,'A','Saved','S','Submitted',flag) flag ,(select evtnme from nomination_admin where evt_id=a.evtnme) event_name from nomination a where evtnme =? and flag <>'X' group by flag,evtnme order by flag desc";
	}	
	psEvt=con.prepareStatement(query);
	psEvt.setString(1,evt_id);
	rsEvt=psEvt.executeQuery();
		while(rsEvt.next()){cnt++;
			...
			if("".equals(dept) && "".equals(townName)){
				evntName=rsEvt.getString("event_name");
			}
		}
...
<%
Boolean show_btns=false;
String dt_chk = "";
psEvt=con.prepareStatement(queryChild);
rsEvt=psEvt.executeQuery();
if(rsEvt.next()){
	dt_chk = rsEvt.getString("chk_evt_dt");
	if(Integer.parseInt(dt_chk)>=20230701){
		show_btns = true;
	}
	else{
		show_btns = false;
...
show_btns=false;
%>
		<%if( (emp.equals("31982600") || emp.equals("31912370")) && show_btns){%>
		<span style="float:Right;margin-right:5%;"> 
			<button class = "btn btn-md btn-primary" onclick = "return send_to_sap();">Send to SAP </button>
 			</span><br>
			<span style="float:left;margin-left:5%;"> 
			<button class = "btn btn-md btn-primary" onclick = "return add_new();">Add New </button>
 			</span>
		<%}%>	
<%%>	

			<span  style="float:left;margin-left:30%;"><a href="viewReport_x.jsp?evt_id=<%="-1".equals(evt_id)?"":evt_id%>&locn=<%="-1".equals(townName)?"":townName%>&dept=<%="-1".equals(dept)?"":dept%>">Summary Report<br><img src="images/excel1.png" title="Download By Employee"></a></span>

			
			<%if(!"".equals(evt_id)){%>
			
		
			<span ><a href="viewReport_excel.jsp?q=<%="-1".equals(evt_id)?"":evt_id%>">Detailed Report<br/><img src="images/excel1.png" width="20" title="Download By Applicant"></a></span>	
			<%}%>
			...
			<%if(!"".equals(evntName)){%>
				<div class="row col-md-12 bg-primary">
					<u class="text-white" style="margin-left:50%;"><center><h5><%=evntName%></h5></center></u></span>
				</div>
			<%}%>
			...
			<table class="table listTable table-hover table-bordered border-primary">
...
<%
Set<String> lblSet1 = new HashSet<String>();
psAdditional = con.prepareStatement(queryAddtionalField1);
psAdditional.setString(1,evt_id);
rsAdditional=psAdditional.executeQuery();
while(rsAdditional.next()){%>
						<th><%=rsAdditional.getString("QUESTIONNAIRE")%><br/>(<%=rsAdditional.getString("QUESTION_TYPE")%>)</th>
						<%if(rsAdditional.getString("QUESTION_TYPE").equals("FILE")){
							lblSet1.add(rsAdditional.getString("id"));
						}%>
<%}%>
				<th>Action</th>
				<th>Status</th>
...
psAdditionalValue1 = con.prepareStatement(queryAdditionlFieldValue1);
psAdditionalValue1.setString(1,evt_id);
...
<%
psAdditionalValue1.setString(2,rsEvt.getString("emp_no"));
rsAdditionalValue1 = psAdditionalValue1.executeQuery();
while(rsAdditionalValue1.next()){%>
						<td>
						<%if(lblSet1.contains((rsAdditionalValue1.getString("id")))){%>
					<a href="useruploads1/<%=rsAdditionalValue1.getString("value")%>" target="_blank"><%=rsAdditionalValue1.getString("value")%></a>
<%}else{%>
					<%=rsAdditionalValue1.getString("value")%>
<%}}%></td>
					<td> 
						<%if( ( emp.equals("31982600") || emp.equals("31912370") ) && show_btns){%>
						<!--<a href="mainSubmit_admin.jsp?evtnme=<%=evt_id%>&editemp=<%=rsEvt.getString("emp_no")%>" class =""><img src="images/edit.png" style="width:15%;height:auto;cursor:pointer"  title="Edit" width="20" ></a>&nbsp; |&nbsp; -->
							<a href="#" onclick='return delete_nom(<%=rsEvt.getString("emp_no")%>);' class =""><img src="images/delete.png" style="width:45%;height:auto;cursor:pointer"  title="Cancel Nomination" ></a> 
						<%}%>
					</td>
				<%
					query = "select * from nomination_sap where event_id=? and emp_no=? and status='S'";
					ps1 = con.prepareStatement(query);
					ps1.setString(1,evt_id);
					ps1.setString(2,rsEvt.getString("emp_no"));
					try{
					...
					<%}
				}catch(Exception ex){out.println("ex="+ex+" & query="+query);}
					%>
					</tr>
<%}
%>				
...
<%} else{%>
	<div style="margin:25px;margin-top:-30px;">
		<center>
			<h3><u>Listing Report</u></h3>
		<div class="table-responsive">
		<table class="table table-hover table-bordered listTable" style="frame:box" >
			<thead class ="alert alert-success">
				<tr>
					<th>&sect;</th>
					<th>Location Code</th>
