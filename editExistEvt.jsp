...
<div class="card-header bg-primary style-app-name"><h3 class="text-white">Edit Event</h3></div>
	<div class="card-body table-responsive">
		<table id="tab1" style="table-layout: fixed;" class="table table-bordered table-hover listTable">
			...
			{evtqry="select to_char(CUTOFDTE,'dd-Mon-yyyy'),MAXTICK,MAXPRICE,EVTNME,EVTPLACE,INDIV_TICK,to_char(evt_date,'dd-Mon-yyyy') evt_date1,EVT_ID,status from nomination_admin where evt_id is not null order by evt_date desc";
					...
					{evtqry="select to_char(CUTOFDTE,'dd-Mon-yyyy'),MAXTICK,MAXPRICE,EVTNME,EVTPLACE,INDIV_TICK,to_char(evt_date,'dd-Mon-yyyy') evt_date1,EVT_ID,status from nomination_admin where   adminemp like '%"+login1+"%' or EVT_CREATED_BY='"+login1+"' order by evt_date desc";
			}
			else		
				{evtqry="select to_char(CUTOFDTE,'dd-Mon-yyyy'),MAXTICK,MAXPRICE,EVTNME,EVTPLACE,INDIV_TICK,to_char(evt_date,'dd-Mon-yyyy') evt_date1,EVT_ID,status from nomination_admin where  regexp_like(adminemp, '"+adminComp+"', 'i') or  adminemp like '%"+login1+"%' or EVT_CREATED_BY='"+login1+"' order by evt_date desc";
			}
			//out.println(evtqry);
			...
			<tr>
				...
				<td>
					...
					Date DisplayForm=new Date("01-Sep-2021");
					// out.println("isTechnicalAdmin-"+isTechnicalAdmin);
					// out.println("DisplayForm-"+DisplayForm);
					// out.println("dbEvtDate-"+dbEvtDate);
					
					// SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
					// Date date1 = sdf.parse("01/09/2021");
					// Date date2 = sdf.parse(dbevtDate);
					...
					<% } else if(isTechnicalAdmin||isProcessAdmin&&dbEvtDate.after(DisplayForm)){%>
