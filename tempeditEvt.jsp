...
PreparedStatement psdet = null,psEvtId=null,psBusReq=null,psEdit = null;
ResultSet rsdet = null,rsEvtId=null,rsBusReq=null,rsEdit = null;
String editQry = "";
int tempMinAge = 0,tempMaxAge=0,sber=0;
boolean isTown =false,isBU =false;
	...
	if(!"".equals(nullVal(request.getParameter("submit1")))){
		...
		if("Add Event".equals(nullVal(request.getParameter("submit1"))) || "Add Event and Publish".equals(nullVal(request.getParameter("submit1")))){
			...
			if("Y".equals(addFields)){
				...
				for(int i=1;i<lblName1.length;i++){
					lblName=lblName1[i];
					lblType=lblType1 [i];
			...
			if("Y".equals(bus_req)){
				...
				if(delCnt>0){
					String tempQry ="insert into nomination_bp values('"+evt_id+"','"+busRequirement+"','"+pickPoints+"')";
	...
	<div class="card md-10 box shadow">
		...
		<tr>
			...
			<td>
				...
				<div id="newDiv" <%=!"Y".equals(busRequirement)?"style='display:none'":""%> ><%=pickPoints%></div>
		...
		<tr>
			...
			<td colspan="3">
				<input type="radio" name="addField" Value="Y" id="Y" onClick="return chkVals(this)" ><label for="Y"> Yes &nbsp;</label>
...
<center>
<%if("".equals(editPara)){%> 
	<button class="btn btn-primary" type="submit" name="submit1" id="submit2" value="Add Event" >Add Event</button>	
	<button type="submit" name="submit1" class="btn btn-primary" id="submit3" id="submit2" value="Add Event and Publish" >Add Event and Publish</button>
<%}else {%>
