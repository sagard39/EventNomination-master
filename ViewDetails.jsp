...
<div>
	<%
		...
		String btnnme = "", btnclkd = "", title = "", ttitle = "", status = "", empBudesc = "", style = "",
				...
				emp_bu = "", loginEmpGrade = "",show="";
		show=nullVal(request.getParameter("View"));
		...
		String evt_cat = "";
		query = "select distinct EVTNME,evt_id from NOMINATION_ADMIN where  (regexp_like(EVTPLACE, trim(?), 'i') or EVTPLACE ='ALL') and  status='P' and emp_age_min<='"+empAge+"' and emp_age_max>='"+empAge+"' order by evt_id";
		psMain = con.prepareStatement(query);
...
<%
 ...
 %> <input type="text"
								class="form-control <%=rsRow.getString("ISMANDATE").equals("Y") ? "isMandate" : ""%>"
								name="addition_Emp_<%=rsRow.getString("id")%>"
								value="<%=nullVal(rsRow.getString("value"))%>" disabled> <%
 	}
 %>
							</td>
						</tr>
					...
					<table class="table table-hover table-bordered listTable">
						<thead class="bg-default alert-primary ">
							<tr>
								...
								<%
								String addQry_emp="select distinct a.evt_id, a.id, b.value, a.lbl_name, a.lbl_type, a.def_value, a.ISMANDATE from nomination_addition a left join nomination_emp_add b on a.id=b.id and b.emp_no=? where a.evt_id=? and a.level_type=? and b.person_code=b.emp_no order by a.id ";
									psRow = con.prepareStatement(addQry_emp);
										psRow.setString(1, emp);
										psRow.setString(2, eventId);
										psRow.setString(3, "A");
										rsRow = psRow.executeQuery();
						...
						<tbody>
							<%
								...
								query="select * from nomination_dependents where event_name=? and  emp_no=? and child_name=emp_no";
								psMain = con.prepareStatement(query);
									psMain.setString(1, eventId);
									psMain.setString(2, emp);
									rsMain = psMain.executeQuery();
									while (rsMain.next()) {
										saved_contact=rsMain.getString("contact_no");
									}
						...
						<tbody>
							<%
								count = 0;
									...
									rsMain = psMain.executeQuery();
									while (rsMain.next()) {
										if(!nullVal(rsMain.getString("event_name")).equals("")){
										count++;
										String relation_string = rsMain.getString("person_code") + "#" + rsMain.getString("relation_code")
												+ "#" + rsMain.getString("gender") + "#" + rsMain.getString("age") + "#"
							...
							&& ("2018_126_event".equals(eventId))) ? "checked" : ""%>
								...
								<%
									...
									String addQry_dep="select distinct a.evt_id, a.id, b.value, a.lbl_name, a.lbl_type, a.def_value, a.ISMANDATE from nomination_addition a left join nomination_emp_add b on a.id=b.id and b.emp_no=? where a.evt_id=? and a.level_type=? and  (b.person_code<>b.emp_no and LENGTH(b.person_code)=8) order by a.id ";
											psRow = con.prepareStatement(addQry_dep);
											psRow.setString(1, emp);
											psRow.setString(2, eventId);
											psRow.setString(3,"A");
											rsRow = psRow.executeQuery();
						...
						<table class="table table-hover table-bordered listTable"
							...
							<thead class="alert alert-success">
								<tr class="lst">
									...
									<% 
									//out.println(addQry);
									String addQry1="select distinct a.evt_id, a.id, b.value, a.lbl_name, a.lbl_type, a.def_value, a.ISMANDATE from nomination_addition a left join nomination_emp_add b on a.id=b.id and b.emp_no=? where a.evt_id=? and a.level_type=? and LENGTH(b.person_code)<8 order by a.id ";
								...
								<tr id="tr<%="#id#".equals(temp) ? "" : temp%>" class="tbodytr"
									...
									<td><center>
											<input type="text" name="person_name_add"
												id="person_name_add<%=temp%>" readonly value="<%=PERSON_CD%>" >
										</center></td>
									<td><center>
											<input type="text" name="rel_add" id="rel_add<%=temp%>"
												value="<%=nullVal(rsMain.getString("relatation"))%>"  disabled />
											<center></td>
									<td><center>
											<input type="number" name="age_rel" maxlength="2"
												id="age_rel<%=temp%>" value="<%=nullVal(rsMain.getString("age"))%>"
												onblur="return ageValid(this)" disabled>
										</center></td>
									<td><select name="gender_rel" disabled id="gender_rel<%=temp%>">
											<option <%=(nullVal(rsMain.getString("gender")).equals("M"))?"selected":"" %> value="M">Male</option>
											<option <%=(nullVal(rsMain.getString("gender")).equals("F"))?"selected":"" %> value="F">Female</option>
									</select></td>
									<td><input type="text" disabled name="contact_rel"
										id="contact_rel<%=temp%>" maxlength="13" value="<%=nullVal(rsMain.getString("contact_no"))%>"></td>
 ...
 %> <%
									...
									%> <textarea cols="100%" rows="2"
										class="select-field <%=rsRow.getString("ISMANDATE").equals("Y") ? "isMandate" : ""%>"
										name="addition_dep_<%=temp%>_<%=rsRow.getString("id")%>"><%=nullVal(rsRow.getString("value"))%></textarea>
									<%
										} else if ("drop down".equals(rsRow.getString("lbl_Type"))) {
									%> <select name="addition_dep_<%=temp%>_<%=rsRow.getString("id")%>"
									class="<%=rsRow.getString("ISMANDATE").equals("Y") ? "isMandate" : ""%>">
										<option value="">Select</option>
										<%
											if (!"".equals(nullVal(rsRow.getString("def_value")))) {
  ...
  <input type="text"
									class="select-field <%=rsRow.getString("ISMANDATE").equals("Y") ? "isMandate" : ""%>"
									name="addition_dep_<%=temp%>_<%=rsRow.getString("id")%>"
									value="<%=nullVal(rsRow.getString("value"))%>" disabled> <%
 	}
 %>
			...
			<div style="margin: 5%">
				...
				<center>
					...
					<%if(!show.equals("view")){%>
					<input disabled type="submit" name="SubmitConfirm"
						<%=("".equals(maxPrize) || "0".equals(maxPrize) || "0.0".equals(maxPrize))?"":"style='display:none'"%>
