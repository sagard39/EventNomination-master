...
String evt_cat="";
query="select distinct EVTNME,evt_id from NOMINATION_ADMIN where  regexp_like(EVTPLACE, trim(?), 'i') and to_date(CUTOFDTE,'dd-mon-yyyy') > = to_date(sysdate,'dd-mon-yyyy') and status='P' order by EVTNME";
...
query="select bu from empmaster where emp_no=?";
psMain=con.prepareStatement(query);
...
query="select distinct EVTNME,evt_id from NOMINATION_ADMIN where  regexp_like(EVTPLACE, trim(?), 'i') and to_date(CUTOFDTE,'dd-mon-yyyy') > = to_date(sysdate,'dd-mon-yyyy') and evt_bu='Y' and status='P' order by EVTNME";
psMain=con.prepareStatement(query);
...
String noOfTicksEmp="",evt_for="",evt_date="",miscVal1="",miscVal2="",miscVal3="",ticktRemaind="",ageCriteria="";
int maxTicksNo=0,tickRemNo=0;
boolean isEmp=false,isDep=false,isOther=false,isAllowed=false;
String flg="";
if(!"".equals(eventId)){
	query="select to_char(CUTOFDTE,'dd-Mon-YYYY') cutOffDate,cast (MAXTICK as int) MAXTICK,MAXPRICE,EVTNME,NOOFMAXTICK,INDIV_TICK,BOARDINGPNT,ADDFIELD,ADMINEMP,EVT_ID,evt_for,bus_facility,to_char(evt_date ,'dd-Mon-yyyy') evt_date,evt_cat,age_criteria from nomination_admin where evt_id=? and cutofdte>=sysdate-1";
	psMain=con.prepareStatement(query);
	...
	query="select count(ticktscnt) numVCl,sum(ticktscnt) tickCount from nomination where evtnme=?";
	psMain=con.prepareStatement(query);
	psMain.setString(1,eventId);
	rsMain=psMain.executeQuery();
	if(rsMain.next()){
	...
	query="select evtnme,flag from nomination where evtnme=? and emp_no=?";
	psMain=con.prepareStatement(query);
	psMain.setString(1,eventId);
		...
		if(isDep && relCode.size()>0){
			for(int i=0;i<relCode.size();i++){
				...
				for(int j=0;j<arrName.length-1;j++){count++;
					...
					psMain.setString(6,eventId);
					psMain.setString(7,emp);
					psMain.setString(8,empContactList.get(i));
					psMain.executeUpdate();	
			}
		}
...
<form name="form1" method="post"  action="mainSubmit.jsp" enctype="multipart/form-data">
	<center><table class="listTable" border="1" style="">
		<tr><td width="30%" class="tdLbl1">Select the Event</td><td>
		...
		<tr>
			<td class="tdLbl1">Date of Event</td><td><input type="text" name="evt_date" value="<%=evt_date%>" readonly></td>
		</tr>
		<tr><td class="tdLbl1">No of Tickets Required</td>
			<td>
			<input type="number" readonly name="noOfTicks" id="tickReq" value="<%=noOfTicks%>" >
			</td>
		</tr>
		<tr><td class="tdLbl1"> Enter the Boarding Point</td>
			<td>
				<select name="bPointName" id="bPointName">
					<option value="-1">Select</option>
					<%
		...
		<tr><td class="tdLbl1">Total Payable Amount</td><td><input type="number" readonly name="pAmount" id="pAmount" value="<%=pAmount%>"  /></td></tr>
<%
int countRow=0;
String addQry="";
//addQry="select a.lbl_name,a.Lbl_type,a.def_value,a.id from nomination_addition a where a.evt_id=?";
addQry="select a.evt_id,a.id,b.value,a.lbl_name,a.lbl_type,a.def_value from nomination_addition a left join nomination_emp_add b on a.id=b.id and b.emp_no=? where  a.evt_id=? order by a.id";
...
while(rsRow.next()){countRow++;%>
		<tr>
			<td class="tdLbl1"><%=rsRow.getString("lbl_Name")%></td>
			<td>
	<%if("file".equals(rsRow.getString("lbl_type"))){%>
				...
				<%if(!"".equals(nullVal(rsRow.getString("value")))){%>
					...
					<input type="hidden" name="hidFilefields<%=rsRow.getString("id")%>" value="<%=nullVal(rsRow.getString("value"))%>" />
				<%}%>
	<%} else if("textarea".equals(rsRow.getString("lbl_type"))){%>

				<textarea cols="100%" rows="2" name="addition<%=rsRow.getString("id")%>"><%=nullVal(rsRow.getString("value"))%></textarea>
	...
	<%}else {
		%>
			...
			<input type="text" name="addition<%=rsRow.getString("id")%>" value="<%=nullVal(rsRow.getString("value"))%>" >
...
<%}%>
		<tr>
			<td class="tdLbl1">Remaining Tickets</td>
			<td><center><span id="remTicks"><%=tickRemNo%></span><b>/</b><%=maxTicksNo%></center></td>
		</tr>
	</table></center><br/>
												<!--Entry for Employee Details Open-->
<%if(isEmp){%>
	<!--<center><font size="4" class="tdLbl1"><b>Employee Details</b></font>-->
	<center><h2>Employee Details</b></h2>
...
%>
		<tr>
			<td><%=count%></td>
			<td><input type="checkbox" name="chkBoxEmp" value="<%=relation_string%>" <%=nominationList.contains(rsMain.getString("person_code"))?"checked":""%> id="chkEmp<%=count%>" onclick="return guidelineFun1(<%=count%>)"></td>
			...
			<td><input type="text" name="emp_contact" id="emp_contact" value="<%=rsMain.getString("contact_no")%>"></td>
		</tr>
<%}%>	
		</tbody>
	</table><br/>
...
<%}if(isDep){%>
	...
	<center><font size="6" class="tdLbl1"><b>Dependents Details</b></font>
	<table class="listTable" border="1">	
		<thead>
			<tr><th>#</th>
			<th>&sect;</th>
...
%>
		<tr>
			<td><%=count%></td>
			<td><input type="checkbox" name="chkBox" value="<%=relation_string%>" <%=nominationList.contains(rsMain.getString("person_code"))?"checked":""%> id="chk<%=count%>" onclick="return guidelineFun(<%=count%>)"></td>
			<td><%=rsMain.getString("person_name")%></td>
			<td><%=rsMain.getString("RELATION_CODE1")%></td>
			<td><%=rsMain.getString("AGE")%></td>
			<td><%=rsMain.getString("rel_dob")%></td>
...
%>		
	<tbody>		
			<tr id="tr<%="#id#".equals(temp)?"":temp%>" class="tbodytr" style='<%="#id#".equals(temp)?"display: none":""%>'>
					...
					<td><center><input type="number" name="age_rel" maxlength="2"   id="age_rel<%=temp%>" value="<%=nullVal(rsMain.getString("age"))%>"></center></td>
					<td>
...
<div style="margin:5%">	
	...
	<center>
	<input type="submit" name="submitEvt" <%="".equals(pAmount)?"style='display:none'":""%>  id="submitEvt" value="Save" onclick="return submitEvtForm();">
	<input type="submit" name="SubmitConfirm" <%="".equals(pAmount)?"style='display:none'":""%> id="submitEvt1" value="Confirm">
	</center>
