						...
						if(dotPos == -1){
							...
							if(!(extension.equalsIgnoreCase("zip")) && !(				extension.equalsIgnoreCase("pdf"))){
								...
								if(name.equals("fileFields0")){
									title = emp+"_Photo"+"."+extension;
									fileFields0 = title;
									fileToCreate = new File(filePath, fileFields0);	
								} if(name.equals("fileFields1")){
									title = emp+"_Id"+"."+extension;
									fileFields1 = title;
									fileToCreate1 = new File(filePath, fileFields1);	
								}if(name.equals("fileFields2")){
									title = emp+"_form"+"."+extension;
									fileFields2 = title;
									fileToCreate2 = new File(filePath, fileFields2);	
								}
								try{
...
if(!("Tata Mumbai Marathon test2".equals(eventname))){%>
<tr>
...
<%if(fieldsName!=null && fieldsName.length>0){%>
	<%for(int i=0;i<fieldsName.length;i++){
		if(!("".equals(fieldsName[i]))){%>
<tr>		
	<td><%=fieldsName[i]%></td><td><input type="file" name="fileFields<%=i%>" value="" /></td>
</tr>
	<%}%>
<%}%>
...
<%if("Tata Mumbai Marathon test1".equals(eventname)){%>
<tr >
<td>Upload the Identity Proof</td><td><input type="file" name="fileFields0" id="fileFields0" value="" ></td>
</tr>
<tr>
<td>Upload the Photograph</td><td><input type="file" name="fileFields1" id="fileFields1" value="" /></td>
</tr>
...
<%}
		...
		if(("submitted").equals(btnclkd)){
			...
			//String exist="select emp_no from nomination where emp_no='"+empn+"' and  evtnme like 'CSR%' and flag <> 'X'";
