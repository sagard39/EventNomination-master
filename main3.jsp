...
$('#confirm').click(function(){ $('div#dialog').dialog('open'); });
 ...
 function proceedAgree(){
	...
	String storepath=storepath1+storepath2;*/
		...
		String depQuery="select person_code,person_name,relation_code,gender,trunc((sysdate-person_dob)/365) age,to_char(person_dob,'dd-Month-yyyy') dob from jdep where emp_no=? and  round(sysdate-person_dob)>8 order by person_code asc";
...
function validate(){
	...
	var chkValue=$('[name="checkValue"]:checked').length;
	var chkTbl=$('[name="depName"]:checked').length;
	var eventname=document.getElementById("eventname").value;
	if(eventname=='Marathon' && chkValue==0){
		alert("Click on View Declaration and agree the Terms and Conditions");
		return false;
	}
	if(eventname=='Marathon' && chkTbl<1){
		alert("Please select altest one name from Dependent List");
		return false;
	}
	/* alert(eventname);
...
<center><h2> <a href="../Marathon/index.jsp" style="color:red"><font size="5">For Reboot@35+ Mumbai - Marathon / Walkathon 2017, click here</font></a></h2></center>
...
<%if("Marathon".equals(eventname)){%>
<center><table border="1" id="depTable" style="border-collapse:collapse;display:none">
	<thead>
		<tr class="listTable">
			<th>&sect;</th>
...
<%
int cnt=0;
while(rsDep.next()){cnt++;%>
		<tr>
			<td><input type="checkbox" name="depName" class="depTbl" id="depCheck<%=rsDep.getString("person_code")%>" value="<%=rsDep.getString("person_code")%>" ></td>
			<td><%=rsDep.getString("Person_name")%>
			
			</td>
			<td><%=rsDep.getString("age")%></td>
			<td><%=rsDep.getString("dob")%></td>
			<td><%=rsDep.getString("relation_code")%>
			<input type="hidden" name="empRel<%=cnt%>" id="empRel<%=cnt%>" value="<%=rsDep.getString("relation_code")%>" />
			</td>	
			<td><%=rsDep.getString("gender")%></td>	
		</tr>
<%}%>	
</tbody>
</table></center>
